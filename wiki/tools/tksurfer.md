...
`tksurfer` was FreeSurfer's original Tk/Tcl-based interactive surface viewer. In FreeSurfer 8.x, `tksurfer` is **deprecated**: the installed binary redirects to `tksurferfv`, which launches [[freeview]] with the appropriate surface loaded. All surface viewing and analysis tasks should now be performed with [[freeview]] directly.
...
- **Note:** In FreeSurfer 8.x, `tksurfer` is a wrapper that calls `tksurferfv`, which launches [[freeview]].
...
All these capabilities are available in [[freeview]], which has replaced `tksurfer`.
...
Since `tksurfer` now redirects to `tksurferfv` (and hence [[freeview]]), see [[tksurferfv]] for flag documentation.
...
`tksurfer` was historically used for surface visualization throughout the pipeline. Modern workflows use [[freeview]].
...
- [[freeview]] — the current FreeSurfer GUI (replaces tksurfer)
