...
| [[annot2std]] | `.annot` | `.annot` | Builds a group-average parcellation in standard ([[fsaverage]]) space by per-vertex voting across mapped subjects |
| [[annot2volseg]] | `.annot` | `.mgz` | General atlas-agnostic counterpart to `mri_aparc2aseg`: maps an annotation onto a subject and fills the cortical ribbon |
| [[aparc_stats_aseg]] | — | `.annot` | Orchestrator that applies a custom `.gcs` atlas (via [[mris_ca_label]]) to reproduce `?h.aparc.<name>.annot` + stats + volume |
| [[wiki/tools/freeview|freeview]] | `.annot` | — | Displays parcellation overlay on surface |
