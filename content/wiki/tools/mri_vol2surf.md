...
| `--ref` | `vol` (string) | `orig.mgz` | Reference volume name used with `--regheader` |
| `--vg-thresh` | `thresh` (float) | `1e-3` | Volume-geometry equality threshold for LTA concatenation checks |
| `--fixtkreg` | — | off | Adjust tkregister matrix so float→int rounding is round-compatible |
| `--nofixtkreg` | — | (default state) | Disable the above |
| `--float2int` | `round\|floor\|tkreg` (string) | `round` | Float-to-int conversion method |
...
| `--srcoldreg` | — | off | Use legacy source registration handling flag (sets `srcoldreg=1`); largely vestigial |
...
| `--surf` | `surfname` (string) | `white` | Surface to sample. Hard-errors if user passes `inflated` here. |
| `--inflated` | — | off | Convenience flag that sets `surfname = inflated` (bypasses the `--surf inflated` block). |
...
| `--cortex` | — | off | Mask output with `?h.cortex.label` of the (target) subject |
...
| Flag | Arguments | Default | Description |
|------|-----------|---------|-------------|
| `--interp` | `nearest\|trilinear` (string) | `nearest` | Interpolation method. The parser only accepts these two strings; the new path additionally supports `cubic` via numeric code in `--vol2surf`. |
| `--float2int` | `round\|floor\|tkreg` (string) | `round` | Float-to-int rounding policy used in nearest-neighbour mode |
...
| `--fwhm` | `mm` (float) | `0` | 3-D Gaussian smoothing FWHM applied to source volume before sampling |
| `--surf-fwhm` | `mm` (float) | `0` | Surface-domain smoothing FWHM applied after sampling |
...
| `--vsm-pedir` | `pedir` (int) | `+2` | Phase-encode direction: `±1=±x`, `±2=±y`, `±3=±z` |
...
| Flag | Arguments | Default | Description |
|------|-----------|---------|-------------|
| `--use-new` | — | off | Force the new resampling path (`MRIvol2surfVSM`); required for `--vsm` and `cubic` interpolation. Default: old path (`UseOld=1`). |
| `--srcsynth` | `seed` (long) | — | Synthesize a random source volume with given seed (testing) |
| `--srcsynth-index` | — | off | Synthesize a source volume containing voxel indices; forces `--interp nearest` |
...
> `mri_vol2surf` does not check for conflicts between `--cortex` and `--mask`. Specifying both is not explicitly blocked but produces undefined masking behaviour. Note: `--mask-to-label` is a flag of [[mri_surf2vol]], not `mri_vol2surf`; do not confuse the two.
...
> [!gap] Audit note: `--targ` and `--volsurf` are not real flags
> These two strings were flagged as potentially missing from the wiki by the C1 audit. Verification against `mri_vol2surf/mri_vol2surf.cpp` confirms: `--targ` does not appear anywhere in the source as a parsed option, and `--volsurf` appears only in a `printf` log message (`printf("mri_vol2surf --volsurf done\n")`) within the `--vol2surf` standalone sub-command handler. Neither is a user-facing flag for `mri_vol2surf`. The correct standalone sub-command is `--vol2surf` (9 args), documented in the Configuration Options table above.
