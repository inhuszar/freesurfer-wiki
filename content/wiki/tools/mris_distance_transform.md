...
|------|----------|---------|-------------|
| `-anterior <y>` | float | -1 | Crop label: keep only vertices anterior to this y-coordinate (mm) |
| `-posterior <y>` | float | -1 | Crop label: keep only vertices posterior to this y-coordinate |
| `-divide <N> <surf>` | int, surf | 1 | Divide label into N parts along primary eigendirection using coords from `<surf>`; nargs=2 |
| `-olabel` | — | off | Write per-division labels rather than distance files (`!stricmp(option, "olabel")`) |
...
| `-signed` | — | — | Positional mode value: signed distance (positive outside, negative inside label) |
| `-unsigned` | — | — | Positional mode value: absolute (unsigned) distance |
| `-outside` | — | — | Positional mode value: distance outside label only; 0 inside |
| `--version` | — | — | Print version and exit |
| `--help` | — | — | Print usage and exit |

> [!note] Mode values as pseudo-flags
> `signed`, `unsigned`, and `outside` are the three values for positional arg 3 (`<mode>`). The audit extractor promotes `stricmp(argv[3], "signed")` etc. to pseudo-flags `-signed`, `-unsigned`, `-outside`.
...
- `-divide` and `-olabel` interact: with `-olabel`, each subdivision is written as a label file rather than a distance curvature; without it, distance values are written.
...
# Divide label into 3 parts and compute separate transforms (using white for division geometry)
mris_distance_transform -divide 3 lh.white lh.white lh.MT.label signed lh.MT.dtrans
