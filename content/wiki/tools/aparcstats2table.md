...
|------|-------|------|---------|-------------|
| `--parc <name>` | `-p` | string | `aparc` | Parcellation name. Determines which stats file is read: `<hemi>.<parc>.stats`. Common alternatives: `aparc.a2009s` (Destrieux), `aparc.DKTatlas`. Any annotation name for which a corresponding `.stats` file exists is accepted. |
| `--measure <meas>` | `-m` | enum | `area` | Measure to extract. Valid: `area`, `volume`, `thickness`, `thickness.T1`, `thicknessstd`, `meancurv`, `gauscurv`, `foldind`, `curvind`. |
