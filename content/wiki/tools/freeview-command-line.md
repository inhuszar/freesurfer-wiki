...
| `loadtrackvolume <FILE>` / `loadvolumetrack <FILE>` | tract volume | Same as `-tv` |
...
| `loadroi <FILE>` / `loadlabel <FILE>` | label file | Same as `-l` |
...
| `setheatscaleoffset <VAL>` / `mean` | float or `mean` | Heat-scale offset |
...
| `gotolabel` / `gotostructure <NAME_OR_VALUE>` | label | Jump slice to best match |
...
|----------|--------|---------|-------------|
| `colormap` / `lut` | `grayscale`, `lut`, `heat`, `jet`, `turbo`/`turboscale`, `gecolor`, `nih`, `pet`, `binary` | `grayscale` | Selects the colour mapping. The Hue colormap is GUI-only. DirectionCoded is DTI-only |
...
| Property | Values | Default | Description |
|----------|--------|---------|-------------|
| `reg` / `transform` | path (`.lta`/`.dat`/`.xfm`) | — | Apply registration transform |
| `resample` / `interpolation` | `nearest`, `trilinear`, `cubic` | `nearest` | Interpolation when resampling. Default is nearest — override globally with `-trilinear`/`-cubic` or locally with this property |
...
| `id` | integer | auto | Numeric layer identifier |
| `lock` / `locked` | 0/1/true/false | 0 | Lock the layer |
...
| `target_surf` / `target` | path | Target surface for vectors / patches to project on |
| `reg` / `affinexfm` | path (`.lta`) | Apply an affine transform to the vertex coordinates (automatically converts to tkRAS) |
...
| `mrisp` / `parameterization_overlay` | path | — | Overlay derived from an MRISP parameterisation |
...
| `annot_outline` / `annotation_outline` / `aparc_outline` | 0/1/true/yes | 0 | Show annotation borders only |
...
| `label_color` / `labelcolor` | colour | auto | Fill colour |
