...
| [[00035]] | JointHisto::smooth — Gaussian kernel uses `exp(−i²/σ²)` instead of standard `exp(−i²/(2σ²))`; missing factor of 2 in the denominator. Achieved smoothing FWHM is `1/√2` of the requested value. Affects `mri_robust_register --cost {MI,NMI,ECC,NCC,SCR}` and `mri_robust_template` cross-modal cost surfaces | medium | open |
| [[00036]] | MRIdilateSegmentation — progressive-dilation loop `while (n != 1)` never exits when `nDils == 0` (decrements 0 → −1 → −2 → … past 1 forever) or any `nDils < −1`. Fires on `mri_convert --dil-seg 0` (a natural "no dilation" reading); the program hangs while applying one dilation per recursive call indefinitely | medium | open |
| [[00037]] | removeRippedVertices (`MRISrenumberRemovingRippedFacesAndVertices`) — compacts only `vertices[]` via `memcpy(sizeof(VERTEX))` and never compacts the parallel `vertices_topology[]` buffer (separately allocated under `SEPARATE_VERTEX_TOPOLOGY`). Topology entry `[i]` is left describing the original vertex at slot `i`, not the compacted survivor; the post-compaction ripflag check `vertices[vno2].ripflag == 1` is also unreliable. Fires on `mris_inflate -explode <thresh>` whenever any vertex is ripped | medium | open |
| [[00038]] | mrisDivideEdge — six-coordinate-triple interpolation block contains a copy-paste duplicate at `:56-58` (verbatim repeat of `:38-40`) where the `whitex/whitey/whitez` triple was meant to be. New vertices added by `MRISdivideLongEdges` carry `whitex = whitey = whitez = 0` (calloc-zeroed). Fires in `mris_make_surfaces` (default recon-all white/pial placement) where `MRISdivideLongEdges` runs after `MRISsaveVertexPositions(WHITE_VERTICES)`; thickness, integration tmp-coords, and pial-placement anchors are all read from the zeroed white-coords | medium | open |
